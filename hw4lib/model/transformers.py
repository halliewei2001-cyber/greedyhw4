...
class EncoderDecoderTransformer(nn.Module):
 ...
 # Positional encoding
        ...
        self.decoder_norm        = nn.LayerNorm(d_model)# Deoder normalization
    ...
    def encode(self, padded_sources: torch.Tensor, source_lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, dict]:
        ...
        pad_mask_src = torch.arange(x_enc.size(1), device=x_enc.device).expand(x_enc.size(0), x_enc.size(1)) >= x_enc_lengths.unsqueeze(1)
    ...
    def decode(
        ...
        pad_mask_tgt = None
        if target_lengths is not None:
            pad_mask_tgt = torch.arange(padded_targets.size(1), device=padded_targets.device).expand(padded_targets.size(0), padded_targets.size(1)) >= target_lengths.unsqueeze(1)
        ...
        causal_mask = torch.triu(torch.ones(padded_targets.size(1), padded_targets.size(1), dtype=torch.bool, device=padded_targets.device), diagonal=1)
        ...
        for i in range(self.num_decoder_layers):
            if self.training and self.layer_drop_rate > 0 and random.random() < self.layer_drop_rate:
                continue
            # TODO: Pass through decoder layer
            x_dec, self_attn, cross_attn = self.dec_layers[i](x_dec, encoder_output, dec_key_padding_mask=pad_mask_tgt, enc_key_padding_mask=pad_mask_src, attn_mask=causal_mask)
